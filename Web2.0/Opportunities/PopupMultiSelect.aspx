	...
	if ( window.opener != null && window.opener.ChangeOpportunity != null )
	{
		...
		for ( var i = 0 ; i < document.all.length ; i++ )
		{
			if ( document.all[i].name == 'chkMain' )
			{
				...
				{
					if ( sOpportunities.length > 0 )
						sOpportunities += ',';
					sOpportunities += document.all[i].value;
				}
			}
		}
		window.opener.ChangeOpportunity(sOpportunities, '');
		window.close();
	}
	else
	{
...
function Cancel()
{
	...
	<asp:Panel ID="pnlPopupButtons" CssClass="button-panel" runat="server">
		<asp:Button ID="btnPopupSelect" OnClientClick="SelectOpportunities(); return false;" CssClass="button" Text='<%# "  " + L10n.Term("Opportunities.LBL_SELECT_CHECKED_BUTTON_LABEL") + "  " %>' ToolTip='<%# L10n.Term("Opportunities.LBL_SELECT_CHECKED_BUTTON_TITLE") %>' runat="server" />

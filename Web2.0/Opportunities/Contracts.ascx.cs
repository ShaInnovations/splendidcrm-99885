...
namespace SplendidCRM.Opportunities
{
	...
	public class Contracts : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                               " + ControlChars.CrLf
				     + "  from vwOPPORTUNITIES_CONTRACTS       " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += " order by CONTRACT_NAME asc            " + ControlChars.CrLf;
					Sql.AddParameter(cmd, "@OPPORTUNITY_ID", gID);

					if ( bDebug )
						RegisterClientScriptBlock("vwOPPORTUNITIES_CONTRACTS", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Opportunities.Contracts");
		}
		#endregion
	}
}

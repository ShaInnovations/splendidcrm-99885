...
namespace SplendidCRM.Opportunities
{
	...
	/// </summary>
	public class MyOpportunities : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                     " + ControlChars.CrLf
				     + "  from vwOPPORTUNITIES_MyList" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += " order by AMOUNT desc" + ControlChars.CrLf;
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataSet ds = new DataSet() )
							{
								using ( DataTable dt = new DataTable("vwOPPORTUNITIES_MyList") )
								{
									...
									da.Fill(ds, 0, 5, "vwOPPORTUNITIES_MyList");
									vwMain = dt.DefaultView;
									grdMain.DataSource = vwMain ;
									if ( !IsPostBack )
									{
										//grdMain.SortColumn = "AMOUNT";
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Opportunities.MyOpportunities");
		}
		#endregion
	}
}

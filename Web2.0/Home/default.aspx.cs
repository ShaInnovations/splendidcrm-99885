...
namespace SplendidCRM.Home
{
	...
	public class Default : SplendidPage
	{
			...
			if ( !IsPostBack )
			{
				...
				if ( Security.IS_ADMIN )
				{
					...
					if ( !Sql.IsEmptyString(sAvailableVersion) )
					{
						...
						{
							lblUpgradeWarning.Text = L10n.Term("Administration.WARN_UPGRADE") + sAvailableVersion + ": " + Sql.ToString(Application["available_version_description"]);
		...
		private void InitializeComponent()
		{
			...
			// 01/17/2008 Paul.  We are replacing WebParts with a DetailView relationship. 
			this.AppendDetailViewRelationships("Home.DetailView.Left" , plcSubPanelLeft );
			this.AppendDetailViewRelationships("Home.DetailView.Body" , plcSubPanelBody );
			this.AppendDetailViewRelationships("Home.DetailView.Right", plcSubPanelRight);
		}
		#endregion
	}
}

...
namespace SplendidCRM.Invoices
{
	...
	public class Activities : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += "   and INVOICE_ID = @INVOICE_ID    " + ControlChars.CrLf;
					cmd.CommandText += " order by DATE_DUE desc          " + ControlChars.CrLf;
					Sql.AddParameter(cmd, "@INVOICE_ID", gID);

					if ( bDebug )
						RegisterClientScriptBlock("vwINVOICES_ACTIVITIES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdOpen   , "Invoices.Activities.Open"   );
			this.AppendGridColumns(grdHistory, "Invoices.Activities.History");
		}
		#endregion
	}
}

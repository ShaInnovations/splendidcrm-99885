			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "BUGS");
						if ( stk.Count > 0 )
						{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spBUGS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.STATUS, ctlMassUpdate.PRIORITY, ctlMassUpdate.RESOLUTION, ctlMassUpdate.TYPE, ctlMassUpdate.SOURCE, ctlMassUpdate.PRODUCT_CATEGORY, ctlMassUpdate.TEAM_ID, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					if ( arrID != null )
					{
						// 10/26/2007 Paul.  Use a stack to run the update in blocks of under 200 IDs. 
						//string sIDs = Utils.ValidateIDs(arrID);
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "delete", arrID, "BUGS");
						if ( stk.Count > 0 )
						{
							DbProviderFactory dbf = DbProviderFactories.GetFactory();
							using ( IDbConnection con = dbf.CreateConnection() )

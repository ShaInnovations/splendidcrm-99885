	...
	public class Threads : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gTHREAD_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spBUGS_THREADS_Delete(gID, gTHREAD_ID);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gTHREAD_ID) )
			{
				try
				{
					SqlProcs.spBUGS_THREADS_Update(gID, gTHREAD_ID);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                       " + ControlChars.CrLf
				     + "  from vwBUGS_THREADS          " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwBUGS_THREADS", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Bugs.Threads");
		}
		#endregion
	}
}

							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
								{
									if ( rdr.Read() )
									{
										...
										string sMESSAGE_URL  = String.Empty;
										if ( bIS_OPTOUT )
										{
											sMESSAGE_URL += "RemoveMe.aspx?identifier=MESSAGE_ID";
										}
										else
										{
											sMESSAGE_URL += "campaign_trackerv2.aspx?track=" + gTRACKER_ID.ToString();
										}
										new DynamicControl(this, "MESSAGE_URL").Text = sSiteURL + sMESSAGE_URL;
									}
									else
									{

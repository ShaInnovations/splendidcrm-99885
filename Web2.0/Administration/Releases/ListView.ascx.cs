...
namespace SplendidCRM.Administration.Releases
{
	...
	public class ListView : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				if ( e.CommandName == "Releases.Create" )
				{
					// 07/04/2007 Paul.  It seemed strange to have the ID in the URL of the previously-edit item when creating a new item. 
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								foreach(DataRow row in dt.Rows)
								{
									...
									row["STATUS"] = L10n.Term(".release_status_dom.", row["STATUS"]);
								}

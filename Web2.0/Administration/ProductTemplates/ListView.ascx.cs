			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "PRODUCT_TEMPLATES");
						if ( stk.Count > 0 )
						{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											//SqlProcs.spPRODUCT_TEMPLATES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.ACCOUNT_ID, ctlMassUpdate.STATUS, T10n.ToServerTime(ctlMassUpdate.START_DATE), T10n.ToServerTime(ctlMassUpdate.END_DATE), T10n.ToServerTime(ctlMassUpdate.COMPANY_SIGNED_DATE), T10n.ToServerTime(ctlMassUpdate.CUSTOMER_SIGNED_DATE), trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					if ( arrID != null )
					{
						// 10/26/2007 Paul.  Use a stack to run the update in blocks of under 200 IDs. 
						//string sIDs = Utils.ValidateIDs(arrID);
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "delete", arrID, "PRODUCT_TEMPLATES");
						if ( stk.Count > 0 )
						{
							DbProviderFactory dbf = DbProviderFactories.GetFactory();
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											//SqlProcs.spPRODUCT_TEMPLATES_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)

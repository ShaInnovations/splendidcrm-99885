...
namespace SplendidCRM.Administration.ForumTopics
{
	...
	public class ListView : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				if ( e.CommandName == "ForumTopics.Create" )
				{
					Response.Redirect("edit.aspx");
				}
				else if ( e.CommandName == "ForumTopics.Edit" )
				{
					Guid gID = Sql.ToGuid(e.CommandArgument);
					Response.Redirect("edit.aspx?ID=" + gID.ToString());
				}
				else if ( e.CommandName == "ForumTopics.Delete" )
				{
					Guid gID = Sql.ToGuid(e.CommandArgument);
					SqlProcs.spFORUM_TOPICS_Delete(gID);
		...
		{
			SetPageTitle(L10n.Term("ForumTopics.LBL_LIST_FORM_TITLE"));

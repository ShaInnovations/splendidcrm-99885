	...
	public class Quotes : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
			{
				try
				{
					SqlProcs.spPROJECT_RELATION_Update(gID, "Quotes", gQUOTE_ID);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                       " + ControlChars.CrLf
				     + "  from vwPROJECTS_QUOTES       " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPROJECTS_QUOTES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Project.Quotes");
		}
		#endregion
	}
}

...
namespace SplendidCRM.Projects
{
	...
	/// </summary>
	public class Threads : SplendidControl
	{
		protected Guid            gID            ;
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gTHREAD_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spPROJECT_THREADS_Delete(gID, gTHREAD_ID);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gTHREAD_ID) )
			{
				try
				{
					SqlProcs.spPROJECT_THREADS_Update(gID, gTHREAD_ID);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
		...
		private void InitializeComponent()
		{
			...
			// 01/02/2008 Paul.  Module name should be Project. 
			this.AppendGridColumns(grdMain, "Project.Threads");
		}
		#endregion
	}
}

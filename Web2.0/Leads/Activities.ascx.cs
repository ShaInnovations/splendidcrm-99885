...
namespace SplendidCRM.Leads
{
	...
	public class Activities : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                 " + ControlChars.CrLf
				     + "  from vwLEADS_ACTIVITIES" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += "   and LEAD_ID = @LEAD_ID" + ControlChars.CrLf;
					cmd.CommandText += " order by DATE_DUE desc  " + ControlChars.CrLf;
					Sql.AddParameter(cmd, "@LEAD_ID", gID);

					if ( bDebug )
						RegisterClientScriptBlock("vwLEADS_ACTIVITIES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdOpen   , "Leads.Activities.Open"   );
			this.AppendGridColumns(grdHistory, "Leads.Activities.History");
		}
		#endregion
	}
}

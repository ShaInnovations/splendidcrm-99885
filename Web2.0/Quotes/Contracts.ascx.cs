...
namespace SplendidCRM.Quotes
{
	...
	public class Contracts : SplendidControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					case "Contracts.Create":
						Response.Redirect("~/Contracts/edit.aspx?QUOTE_ID=" + gID.ToString());
						break;
					case "Contracts.Edit":
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwQUOTES_CONTRACTS", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Quotes.Contracts");
		}
		#endregion
	}
}

...
<div id="divNewRecord">
	...
	<SplendidCRM:HeaderLeft ID="ctlHeaderLeft" Title="Cases.LBL_NEW_FORM_TITLE" Runat="Server" />
	...
	<asp:Panel Width="100%" CssClass="leftColumnModuleS3" runat="server">
		<asp:Label       ID="lblNAME"         Text='<%# L10n.Term("Cases.LBL_SUBJECT") %>' runat="server" />&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br />
		<asp:TextBox     ID="txtNAME"         size="27" MaxLength="255" Runat="server" /><br />
		<asp:Label       ID="lblACCOUNT_NAME" Text='<%# L10n.Term("Cases.LBL_ACCOUNT_NAME") %>' runat="server" />&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br />
		<asp:TextBox     ID="txtACCOUNT_NAME" ReadOnly="True" size="12" Runat="server" />
		...
		<SplendidCRM:RequiredFieldValidatorForHiddenInputs ID="reqACCOUNT_ID" ControlToValidate="txtACCOUNT_ID" CssClass="required" Enabled="false" EnableClientScript="false" EnableViewState="false" Runat="server" />
		<asp:Label ID="lblError" ForeColor="Red" EnableViewState="false" Runat="server" />
	</asp:Panel>
	<%= Utils.RegisterEnterKeyPress(txtNAME.ClientID        , btnSave.ClientID) %>
	<%= Utils.RegisterEnterKeyPress(txtACCOUNT_NAME.ClientID, btnSave.ClientID) %>
</div>

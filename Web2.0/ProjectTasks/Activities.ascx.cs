...
namespace SplendidCRM.ProjectTasks
{
	...
	public class Activities : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                         " + ControlChars.CrLf
				     + "  from vwPROJECT_TASKS_ACTIVITIES" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += "   and PROJECT_TASK_ID = @PROJECT_TASK_ID" + ControlChars.CrLf;
					cmd.CommandText += " order by DATE_DUE desc                  " + ControlChars.CrLf;
					Sql.AddParameter(cmd, "@PROJECT_TASK_ID", gID);

					if ( bDebug )
						RegisterClientScriptBlock("vwPROJECT_TASKS_ACTIVITIES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdOpen   , "ProjectTask.Activities.Open"   );
			this.AppendGridColumns(grdHistory, "ProjectTask.Activities.History");
		}
		#endregion
	}
}

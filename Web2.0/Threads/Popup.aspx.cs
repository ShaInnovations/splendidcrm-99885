...
namespace SplendidCRM.Threads
{
	...
	public class Popup : SplendidPage
	{
		...
		{
			SetPageTitle(L10n.Term("Threads.LBL_LIST_FORM_TITLE"));
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select *                   " + ControlChars.CrLf
					     + "  from vwTHREADS_List      " + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						// 11/24/2006 Paul.  Use new Security.Filter() function to apply Team and ACL security rules.
						Security.Filter(cmd, "Threads", "list");
		...
		{    
			...
			this.AppendGridColumns(grdMain, "Threads.PopupView");
		}
		#endregion
	}
}

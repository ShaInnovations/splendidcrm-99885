...
namespace SplendidCRM.Prospects
{
	...
	public class Activities : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                     " + ControlChars.CrLf
				     + "  from vwPROSPECTS_ACTIVITIES" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += "   and PROSPECT_ID = @PROSPECT_ID" + ControlChars.CrLf;
					cmd.CommandText += " order by DATE_DUE desc  " + ControlChars.CrLf;
					Sql.AddParameter(cmd, "@PROSPECT_ID", gID);

					if ( bDebug )
						RegisterClientScriptBlock("vwPROSPECTS_ACTIVITIES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdOpen   , "Prospects.Activities.Open"   );
			this.AppendGridColumns(grdHistory, "Prospects.Activities.History");
		}
		#endregion
	}
}

...
namespace SplendidCRM.Forums
{
	...
	/// </summary>
	public class Posts : SplendidControl
	{
		protected Guid            gID            ;
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					case "Posts.Create":
						Response.Redirect("~/Posts/edit.aspx?FORUM_ID=" + gID.ToString());
						break;
					case "Posts.View":
					{
						Guid gTHREAD_ID = Sql.ToGuid(e.CommandArgument);
						Response.Redirect("~/Posts/view.aspx?ID=" + gTHREAD_ID.ToString());
						break;
					}
					default:
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					cmd.CommandText += "   and FORUM_ID = @FORUM_ID" + ControlChars.CrLf;
					cmd.CommandText += " order by DATE_ENTERED desc" + ControlChars.CrLf;
					Sql.AddParameter(cmd, "@FORUM_ID", gID);

					if ( bDebug )
						RegisterClientScriptBlock("vwFORUMS_THREADS", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Forums.Posts");
		}
		#endregion
	}
}

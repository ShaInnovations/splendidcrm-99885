		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                  " + ControlChars.CrLf
				     + "  from vwORDERS_LINE_ITEMS" + ControlChars.CrLf
				     + " where 1 = 1              " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Sql.AppendParameter(cmd, gID, "ORDER_ID", false);
					cmd.CommandText += " order by POSITION asc" + ControlChars.CrLf;

					if ( bDebug )
						RegisterClientScriptBlock("vwORDERS_LINE_ITEMS", Sql.ClientScriptBlock(cmd));

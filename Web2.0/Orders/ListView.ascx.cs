...
namespace SplendidCRM.Orders
{
	...
	{
		protected SplendidCRM._controls.ExportHeader ctlExportHeader;
			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "ORDERS");
						if ( stk.Count > 0 )
						{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spORDERS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.PAYMENT_TERMS, ctlMassUpdate.ORDER_STAGE, T10n.ToServerTime(ctlMassUpdate.DATE_ORDER_DUE), T10n.ToServerTime(ctlMassUpdate.DATE_ORDER_SHIPPED), ctlMassUpdate.TEAM_ID, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					if ( arrID != null )
					{
						// 10/26/2007 Paul.  Use a stack to run the update in blocks of under 200 IDs. 
						//string sIDs = Utils.ValidateIDs(arrID);
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "delete", arrID, "ORDERS");
						if ( stk.Count > 0 )
						{
							DbProviderFactory dbf = DbProviderFactories.GetFactory();
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spORDERS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)

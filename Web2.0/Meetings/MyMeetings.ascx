...
<div id="divMeetingsMyMeetings">
	...
	<SplendidCRM:ListHeader Title="Meetings.LBL_LIST_MY_MEETINGS" Runat="Server" />
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
		...
		<Columns>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Center">
				<ItemTemplate>
					<asp:Image ImageUrl='<%# Session["themeURL"] + "images/Meetings.gif" %>' BorderWidth="0" Width="16" Height="16" ImageAlign="AbsMiddle" Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:TemplateColumn HeaderText="Activities.LBL_LIST_CLOSE" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Center">
				<ItemTemplate>
					<asp:HyperLink Visible='<%# SplendidCRM.Security.GetUserAccess("Meetings", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID"))) >= 0 %>' NavigateUrl='<%# "~/Meetings/edit.aspx?id=" + DataBinder.Eval(Container.DataItem, "ID") + "&Status=Close" %>' Runat="server">
			...
			<asp:TemplateColumn  HeaderText="Meetings.LBL_LIST_DATE" SortExpression="DATE_START" ItemStyle-Width="10%" ItemStyle-Wrap="false">
				<ItemTemplate>
			...
			<asp:TemplateColumn HeaderText="Activities.LBL_ACCEPT_THIS" ItemStyle-Width="10%" ItemStyle-Wrap="false">
				<ItemTemplate>
					<div style="DISPLAY: <%# String.Compare((DataBinder.Eval(Container.DataItem, "ACCEPT_STATUS") as string), "none", true) == 0 ? "inline" : "none" %>">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Meetings", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Activity.Accept"    CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" ImageUrl='<%# Session["themeURL"] + "images/accept_inline.gif"    %>' AlternateText='<%# L10n.Term(".dom_meeting_accept_options.accept"   ) %>' BorderWidth="0" Width="12" Height="14" ImageAlign="AbsMiddle" Runat="server" />
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Meetings", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Activity.Tentative" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" ImageUrl='<%# Session["themeURL"] + "images/tentative_inline.gif" %>' AlternateText='<%# L10n.Term(".dom_meeting_accept_options.tentative") %>' BorderWidth="0" Width="12" Height="14" ImageAlign="AbsMiddle" Runat="server" />
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Meetings", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Activity.Decline"   CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" ImageUrl='<%# Session["themeURL"] + "images/decline_inline.gif"   %>' AlternateText='<%# L10n.Term(".dom_meeting_accept_options.decline"  ) %>' BorderWidth="0" Width="12" Height="14" ImageAlign="AbsMiddle" Runat="server" />
					</div>
					<div style="DISPLAY: <%# String.Compare((DataBinder.Eval(Container.DataItem, "ACCEPT_STATUS") as string), "none", true) != 0 ? "inline" : "none" %>">

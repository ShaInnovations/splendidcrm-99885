...
{
	return window.open('../Opportunities/Popup.aspx?ClearDisabled=1','OpportunityPopup','width=600,height=400,resizable=1,scrollbars=1');
}
</script>
<div id="divEmailsOpportunities">
	...
	<asp:Panel CssClass="button-panel" Visible="<%# !PrintView %>" runat="server">
		<asp:Button Visible='<%# SplendidCRM.Security.GetUserAccess("Opportunities", "edit") >= 0 %>' CommandName="Opportunities.Create" OnCommand="Page_Command" CssClass="button" Text='<%# "  " + L10n.Term(".LBL_NEW_BUTTON_LABEL"   ) + "  " %>' ToolTip='<%# L10n.Term(".LBL_NEW_BUTTON_TITLE"   ) %>' AccessKey='<%# L10n.AccessKey(".LBL_NEW_BUTTON_KEY"   ) %>' Runat="server" />
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Emails.NTC_REMOVE_INVITEE") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Emails", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "EMAIL_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Opportunities.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "OPPORTUNITY_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' ImageUrl='<%# Session["themeURL"] + "images/delete_inline.gif" %>' BorderWidth="0" Width="12" Height="12" ImageAlign="AbsMiddle" Runat="server" />

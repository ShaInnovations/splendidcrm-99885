	...
	public class Quotes : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gQUOTE_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spEMAILS_QUOTES_Delete(gID, gQUOTE_ID);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
			{
				try
				{
					SqlProcs.spEMAILS_QUOTES_Update(gID, gQUOTE_ID);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                     " + ControlChars.CrLf
				     + "  from vwEMAILS_QUOTES       " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwEMAILS_QUOTES", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Emails.Quotes");
		}
		#endregion
	}
}

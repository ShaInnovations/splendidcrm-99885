...
namespace SplendidCRM.Payments
{
	...
	public class ListView : SplendidControl
	{
			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "PAYMENTS");
						if ( stk.Count > 0 )
						{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spPAYMENTS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, T10n.ToServerTime(ctlMassUpdate.PAYMENT_DATE), ctlMassUpdate.PAYMENT_TYPE, ctlMassUpdate.TEAM_ID, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					if ( arrID != null )
					{
						// 10/26/2007 Paul.  Use a stack to run the update in blocks of under 200 IDs. 
						//string sIDs = Utils.ValidateIDs(arrID);
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "delete", arrID, "PAYMENTS");
						if ( stk.Count > 0 )
						{
							DbProviderFactory dbf = DbProviderFactories.GetFactory();
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = con.BeginTransaction() )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											//SqlProcs.spPAYMENTS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select *              " + ControlChars.CrLf
					     + "  from vwPAYMENTS_List" + ControlChars.CrLf;
						...
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								if ( !IsPostBack )
								{
									...
									{
										grdMain.SortColumn = "PAYMENT_DATE";
		...
		{
			//
			// CODEGEN: This Payment is required by the ASP.NET Web Form Designer.
			//
			InitializeComponent();
			base.OnInit(e);
		}

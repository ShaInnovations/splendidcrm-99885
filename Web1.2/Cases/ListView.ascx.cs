			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "CASES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spCASES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.ACCOUNT_ID, ctlMassUpdate.STATUS, ctlMassUpdate.PRIORITY);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "CASES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spCASES_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

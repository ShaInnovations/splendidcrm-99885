...
namespace SplendidCRM.ProspectLists
{
	...
	/// </summary>
	public class Leads : SplendidControl
	{
		protected Guid            gID            ;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyString(txtLEAD_ID.Value) )
			{
				...
				if ( !Sql.IsEmptyString(sIDs) )
				{
					try
					{
						SqlProcs.spPROSPECT_LISTS_LEADS_MassUpdate(sIDs, gID);
						Response.Redirect("view.aspx?ID=" + gID.ToString());
					}
					catch(Exception ex)
					{
...
#if DEBUG
					Page.RegisterClientScriptBlock("vwPROSPECT_LISTS_LEADS", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("ProspectLists.Leads");
		}
		#endregion
	}
}

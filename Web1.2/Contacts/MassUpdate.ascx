	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			<td style="padding-bottom: 2px;">
				...
				<asp:Button ID="btnSync"   CommandName="MassUpdate" OnCommand="Page_Command" CssClass="button" Text='<%# L10n.Term(".LBL_SYNC"  ) %>' Runat="server" />
				<asp:Button ID="btnUnsync" CommandName="MassDelete" OnCommand="Page_Command" CssClass="button" Text='<%# L10n.Term(".LBL_UNSYNC") %>' Runat="server" />
			</td>
		</tr>
	</table>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="tabForm">
		<tr>
			<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="15%" class="dataLabel"><%= L10n.Term("Contacts.LBL_LEAD_SOURCE") %></td>
						<td width="35%" class="dataField"><asp:DropDownList ID="lstLEAD_SOURCE" DataValueField="NAME" DataTextField="DISPLAY_NAME" Runat="server" /></td>
						<td width="15%" class="dataLabel"><%= L10n.Term("Contacts.LBL_ACCOUNT_NAME") %></td>
						<td width="35%" class="dataField">
							...
							<input ID="btnChangeAccount" type="button" class="button" onclick="return AccountPopup();" title="<%# L10n.Term(".LBL_CHANGE_BUTTON_TITLE") %>" accessKey="<%# L10n.Term(".LBL_CHANGE_BUTTON_KEY") %>" value="<%# L10n.Term(".LBL_CHANGE_BUTTON_LABEL") %>" />
						</td>
					</tr>
					<tr>
						<td class="dataLabel"><%= L10n.Term(".LBL_ASSIGNED_TO") %></td>
						<td class="dataField"><asp:DropDownList ID="lstASSIGNED_USER_ID" DataValueField="ID" DataTextField="USER_NAME" Runat="server" /></td>
						<td class="dataLabel"><%= L10n.Term("Contacts.LBL_REPORTS_TO") %></td>
						<td class="dataField">
							<asp:TextBox ID="txtREPORTS_TO_NAME" ReadOnly="True" Runat="server" />
							<input ID="txtREPORTS_TO_ID" type="hidden" runat="server" />

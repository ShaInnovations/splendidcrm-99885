...
namespace SplendidCRM.Contacts
{
	...
	public class DirectReports : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                        " + ControlChars.CrLf
				     ...
				     + " order by LAST_NAME asc         " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Sql.AddParameter(cmd, "@CONTACT_ID", gID);
#if DEBUG
					Page.RegisterClientScriptBlock("vwCONTACTS_DIRECT_REPORTS", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("Contacts.DirectReports");
		}
		#endregion
	}
}

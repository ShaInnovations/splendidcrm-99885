<%@ Page language="c#" Codebehind="Popup.aspx.cs" AutoEventWireup="false" Inherits="SplendidCRM.Bugs.Popup" %>
...
function CancelBug()
{
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-bottom: 2px;">
				...
				<input id="btnClear" type="button" class="button" onclick="ClearBug();"  value='<%= "  " + L10n.Term(".LBL_CLEAR_BUTTON_LABEL" ) + "  " %>' title='<%= L10n.Term(".LBL_CLEAR_BUTTON_TITLE" ) %>' AccessKey='<%= L10n.Term(".LBL_CLEAR_BUTTON_KEY" ) %>' />
				</div>
				<input id="btnCancel" type="button" class="button" onclick="CancelBug();" value='<%= "  " + L10n.Term(".LBL_CANCEL_BUTTON_LABEL") + "  " %>' title='<%= L10n.Term(".LBL_CANCEL_BUTTON_TITLE") %>' AccessKey='<%= L10n.Term(".LBL_CANCEL_BUTTON_KEY") %>' />
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
		...
		<Columns>
			...
			<asp:TemplateColumn HeaderText="Bugs.LBL_LIST_SUBJECT"                                    SortExpression="NAME"         ItemStyle-Width="40%">
				<ItemTemplate>
					<a name="BUG_ID" id="BUG_ID_<%# DataBinder.Eval(Container.DataItem, "ID") %>" class="listViewTdLinkS1" href="#" onclick="SelectBug('<%# Sql.EscapeJavaScript(Sql.ToString(DataBinder.Eval(Container.DataItem, "ID"))) %>', '<%# Sql.EscapeJavaScript(Sql.ToString(DataBinder.Eval(Container.DataItem, "NAME"))) %>');"><%# DataBinder.Eval(Container.DataItem, "NAME") %></a>
				</ItemTemplate>
			</asp:TemplateColumn>

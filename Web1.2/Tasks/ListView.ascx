...
<div id="divListView">
	...
	<SplendidCRM:ModuleHeader ID="ctlModuleHeader" Module="Tasks" Title=".moduleList.Home" EnablePrint="true" EnableHelp="true" Runat="Server" />
	<%@ Register TagPrefix="SplendidCRM" Tagname="ListHeader" Src="~/_controls/ListHeader.ascx" %>
	<SplendidCRM:ListHeader Visible="<%# !PrintView %>" Title="Tasks.LBL_SEARCH_FORM_TITLE" Runat="Server" />
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
		...
		<Columns>
			...
			<asp:TemplateColumn HeaderText="Tasks.LBL_LIST_CLOSE" SortExpression="STATUS" ItemStyle-Width="1%">
				<ItemTemplate>
					<asp:ImageButton CommandName="Task.Close" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' Visible='<%# Sql.ToBoolean(DataBinder.Eval(Container.DataItem, "CAN_CLOSE")) %>' OnCommand="Page_Command" ImageUrl='<%# Session["themeURL"] + "images/close_inline.gif" %>' AlternateText='<%# L10n.Term("Tasks.LBL_LIST_CLOSE") %>' BorderWidth="0" Width="12" Height="14" ImageAlign="AbsMiddle" Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>

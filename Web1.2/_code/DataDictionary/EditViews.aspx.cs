...
namespace SplendidCRM._code.DataDictionary
{
	/// <summary>
	/// Summary description for EditViews.
	/// </summary>
	public class EditViews : System.Web.UI.Page
	{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							if ( Sql.IsEmptyString(sNAME) )
							{
								...
								{
									Response.Write("<html><body><h1>EditViews</h1>");
									while ( rdr.Read() )
									{
										Response.Write("<a href=\"EditViews.aspx?NAME=" + rdr.GetString(rdr.GetOrdinal("NAME")) + "\">" + rdr.GetString(rdr.GetOrdinal("NAME")) + "</a><br>" + ControlChars.CrLf);
									}
							...
							else
							{
								...
								sSQL = "select DATA_FIELD               " + ControlChars.CrLf
								     + "     , DISPLAY_FIELD            " + ControlChars.CrLf
								...
								using ( SqlDataReader rdr = (SqlDataReader) cmd.ExecuteReader() )
								{
									while ( rdr.Read() )
									{
										...
										Utils.AppendDictionaryEntry(xml, sDATA_FIELD, "ctlEditView_" + sDATA_FIELD);

										if ( !rdr.IsDBNull(rdr.GetOrdinal("DISPLAY_FIELD")) )
										{
											string sDISPLAY_FIELD = rdr.GetString(rdr.GetOrdinal("DISPLAY_FIELD"));
											if ( sDATA_FIELD != sDISPLAY_FIELD )
												Utils.AppendDictionaryEntry(xml, sDISPLAY_FIELD, "ctlEditView_" + sDISPLAY_FIELD);
										}
									}
								}
								Utils.AppendDictionaryEntry(xml, "btnSave"  , "ctlEditView_ctlEditButtons_btnSave"  );
								Utils.AppendDictionaryEntry(xml, "btnCancel", "ctlEditView_ctlEditButtons_btnCancel");

...
{
	/// <summary>
	/// Summary description for DetailViews.
	/// </summary>
	public class DetailViews : System.Web.UI.Page
	{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							if ( Sql.IsEmptyString(sNAME) )
							{
								...
								{
									Response.Write("<html><body><h1>DetailViews</h1>");
									while ( rdr.Read() )
									{
										Response.Write("<a href=\"DetailViews.aspx?NAME=" + rdr.GetString(rdr.GetOrdinal("NAME")) + "\">" + rdr.GetString(rdr.GetOrdinal("NAME")) + "</a><br>" + ControlChars.CrLf);
									}
							...
							else
							{
								...
								using ( SqlDataReader rdr = (SqlDataReader) cmd.ExecuteReader() )
								{
									while ( rdr.Read() )
									{
										...
										Utils.AppendDictionaryEntry(xml, sDATA_FIELD, "ctlDetailView_" + sDATA_FIELD);
										...
										if ( !rdr.IsDBNull(rdr.GetOrdinal("URL_FIELD")) )
										{
											...
											if ( sDATA_FIELD != sURL_FIELD )
												Utils.AppendDictionaryEntry(xml, sURL_FIELD, "ctlDetailView_" + sURL_FIELD);
										}
									}
								}
								Utils.AppendDictionaryEntry(xml, "btnEdit"     , "ctlDetailView_ctlEditButtons_btnEdit"     );
								Utils.AppendDictionaryEntry(xml, "btnDuplicate", "ctlDetailView_ctlEditButtons_btnDuplicate");
								Utils.AppendDictionaryEntry(xml, "btnDelete"   , "ctlDetailView_ctlEditButtons_btnDelete"   );

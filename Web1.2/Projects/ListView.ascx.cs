			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "PROJECT");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spPROJECTS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "PROJECT");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spPROJECTS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

	...
	public class Contacts : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gCONTACT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spPROJECT_RELATION_Delete(gID, gCONTACT_ID);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
			{
				try
				{
					SqlProcs.spPROJECT_RELATION_Update(gID, "Contacts", gCONTACT_ID);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
...
#if DEBUG
					Page.RegisterClientScriptBlock("vwPROJECTS_CONTACTS", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("Project.Contacts");
		}
		#endregion
	}
}

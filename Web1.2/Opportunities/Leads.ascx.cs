...
namespace SplendidCRM.Opportunities
{
	...
	public class Leads : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                               " + ControlChars.CrLf
				     ...
				     + " order by LEAD_NAME asc                " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Sql.AddParameter(cmd, "@OPPORTUNITY_ID", gID);
#if DEBUG
					Page.RegisterClientScriptBlock("vwOPPORTUNITIES_LEADS", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("Opportunities.Leads");
		}
		#endregion
	}
}

					...
					if ( arrLEAD_SOURCE != null )
					{
						foreach(string sLEAD_SOURCE in arrLEAD_SOURCE)
						{
							...
							if ( sLEAD_SOURCE == String.Empty )
								XmlUtil.SetSingleNodeAttribute(xml, nodeRow, "title"   , Sql.ToString(L10n.Term(".lead_source_dom.", "None")));
							else
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader rdr = cmd.ExecuteReader() )
						{
							...
							while ( rdr.Read() )
							{
								...
								if ( sLEAD_SOURCE == String.Empty )
									sLEAD_SOURCE = "None";
								XmlNode nodeRow = nodeYData.SelectSingleNode("dataRow[@title=\'" + Sql.ToString(L10n.Term(".lead_source_dom.", sLEAD_SOURCE)).Replace("'", "\'") +"\']");
								if ( nodeRow == null )
								{

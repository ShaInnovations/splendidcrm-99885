<%@ Page language="c#" Codebehind="PopupMultiSelect.aspx.cs" AutoEventWireup="false" Inherits="SplendidCRM.Prospects.PopupMultiSelect" %>
	...
	if ( window.opener != null && window.opener.ChangeProspect != null )
	{
		...
		for ( var i = 0 ; i < document.all.length ; i++ )
		{
			if ( document.all[i].name == 'chkMain' )
			{
				...
				{
					if ( sProspects.length > 0 )
						sProspects += ',';
					sProspects += document.all[i].value;
				}
			}
		}
		window.opener.ChangeProspect(sProspects, '');
		window.close();
	}
	else
	{
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-bottom: 2px;">
				<input id="btnSelect" type="button" class="button" onclick="SelectProspects();" value='<%= "  " + L10n.Term("Prospects.LBL_SELECT_CHECKED_BUTTON_LABEL") + "  " %>' title='<%= L10n.Term("Prospects.LBL_SELECT_CHECKED_BUTTON_TITLE") %>' />
				<input id="btnCancel" type="button" class="button" onclick="CancelProspects();" value='<%= "  " + L10n.Term(".LBL_DONE_BUTTON_LABEL"                   ) + "  " %>' title='<%= L10n.Term(".LBL_DONE_BUTTON_TITLE"                   ) %>' AccessKey='<%= L10n.Term(".LBL_DONE_BUTTON_KEY") %>' />
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
		...
		<Columns>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="2%">
				<ItemTemplate>
					<input name="chkMain" id="PROSPECT_ID_<%# DataBinder.Eval(Container.DataItem, "ID") %>" class="checkbox" type="checkbox" value="<%# DataBinder.Eval(Container.DataItem, "ID") %>" />
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:BoundColumn     HeaderText="Prospects.LBL_LIST_NAME"          DataField="NAME"             SortExpression="NAME"         ItemStyle-Width="34%" />
			<asp:BoundColumn     HeaderText="Prospects.LBL_LIST_TITLE"         DataField="TITLE"            SortExpression="TITLE"        ItemStyle-Width="15%" />
			<asp:BoundColumn     HeaderText="Prospects.LBL_LIST_EMAIL_ADDRESS" DataField="EMAIL1"           SortExpression="EMAIL1"       ItemStyle-Width="34%" />

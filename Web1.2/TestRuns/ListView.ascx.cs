...
namespace SplendidCRM.TestRuns
{
	/// <summary>
			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "TEST_RUNS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spTEST_RUNS_MassUpdate(sIDs, ctlMassUpdate.STATUS);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "TEST_RUNS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spTEST_RUNS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select *               " + ControlChars.CrLf
					     + "  from vwTEST_RUNS_List" + ControlChars.CrLf

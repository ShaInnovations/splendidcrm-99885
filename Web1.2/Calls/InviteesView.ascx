		...
		<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
			...
			<Columns>
				...
				<asp:TemplateColumn HeaderText="" ItemStyle-HorizontalAlign="Center">
					<ItemTemplate>
						<asp:Button CommandName="Invitees.Add" OnCommand="Page_Command" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' CssClass="button" Text='<%# " " + L10n.Term("Calls.LBL_ADD_BUTTON") + " " %>' title='<%# L10n.Term("Calls.LBL_ADD_BUTTON") %>' Enabled='<%# !IsExistingInvitee(DataBinder.Eval(Container.DataItem, "ID").ToString()) %>' Runat="server" />
					</ItemTemplate>
				</asp:TemplateColumn>
			</Columns>

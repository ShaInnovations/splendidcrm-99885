...
namespace SplendidCRM.Meetings
{
	...
	public class Contacts : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                       " + ControlChars.CrLf
				     ...
				     + " where MEETING_ID = @MEETING_ID" + ControlChars.CrLf
				     + " order by DATE_ENTERED asc     " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Sql.AddParameter(cmd, "@MEETING_ID", gID);
#if DEBUG
					Page.RegisterClientScriptBlock("vwMEETINGS_CONTACTS", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("Meetings.Contacts");
		}
		#endregion
	}
}

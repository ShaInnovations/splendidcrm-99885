			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "MEETINGS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							// 07/09/2006 Paul.  The date conversion was moved out of the MassUpdate control. 
							SqlProcs.spMEETINGS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, T10n.ToServerTime(ctlMassUpdate.DATE_START), ctlMassUpdate.STATUS);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "MEETINGS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spMEETINGS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

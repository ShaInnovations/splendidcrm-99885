			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "TEST_CASES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spTEST_CASES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.TEST_TYPE, ctlMassUpdate.TEST_PHASE);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "TEST_CASES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spTEST_CASES_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select *                " + ControlChars.CrLf
					     + "  from vwTEST_CASES_List" + ControlChars.CrLf

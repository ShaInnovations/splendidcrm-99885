...
namespace SplendidCRM.TestCases
{
	...
	public class NewRecord : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "NewRecord" )
			{
				...
				if ( Page.IsValid )
				{
					...
					try
					{
						SqlProcs.spTEST_CASES_New(ref gID, Sql.ToGuid(lstASSIGNED_USER_ID.SelectedValue), txtNAME.Text);
					}
					catch(Exception ex)
					{
					...
					if ( !Sql.IsEmptyGuid(gID) )
						Response.Redirect("~/TestCases/view.aspx?ID=" + gID.ToString());
				}
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			reqNAME.ErrorMessage = L10n.Term(".ERR_MISSING_REQUIRED_FIELDS") + " " + L10n.Term("TestCases.LBL_LIST_NAME") + "<br>";
			if ( !IsPostBack )
			{

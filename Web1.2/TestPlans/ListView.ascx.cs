...
namespace SplendidCRM.TestPlans
{
	/// <summary>
			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "TEST_PLANS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spTEST_PLANS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.STATUS, ctlMassUpdate.TEST_PLAN_TYPE);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "TEST_PLANS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spTEST_PLANS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select *                " + ControlChars.CrLf
					     + "  from vwTEST_PLANS_List" + ControlChars.CrLf

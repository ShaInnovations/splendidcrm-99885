			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "LEADS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spLEADS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.LEAD_SOURCE, ctlMassUpdate.STATUS, ctlMassUpdate.REPORTS_TO_ID);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "LEADS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spLEADS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

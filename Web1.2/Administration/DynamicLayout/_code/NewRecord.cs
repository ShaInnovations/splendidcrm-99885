...
namespace SplendidCRM.Administration.DynamicLayout
{
	...
	public class NewRecord : SplendidControl
	{
		...
		protected HtmlInputHidden    txtFIELD_ID       ;
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								row1["LIST_NAME"] = "AssignedUser";
								row2["LIST_NAME"] = "Currencies"  ;
								row3["LIST_NAME"] = "Release"     ;
								dt.Rows.InsertAt(row0, 0);
								dt.Rows.InsertAt(row1, 1);
								dt.Rows.InsertAt(row2, 2);
								dt.Rows.InsertAt(row3, 3);

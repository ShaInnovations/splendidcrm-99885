<%@ Page language="c#" Codebehind="Popup.aspx.cs" AutoEventWireup="false" Inherits="SplendidCRM.Administration.ProductCategories.Popup" %>
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
		...
		<Columns>
			<asp:TemplateColumn HeaderText="ProductCategories.LBL_LIST_NAME" SortExpression="NAME" ItemStyle-Width="40%">
				<ItemTemplate>
					<a name="ID" id="ID_<%# DataBinder.Eval(Container.DataItem, "ID") %>" class="listViewTdLinkS1" href="#" onclick="SelectProductCategory('<%# Sql.EscapeJavaScript(Sql.ToString(DataBinder.Eval(Container.DataItem, "ID"))) %>', '<%# Sql.EscapeJavaScript(Sql.ToString(DataBinder.Eval(Container.DataItem, "NAME"))) %>');"><%# DataBinder.Eval(Container.DataItem, "NAME") %></a>
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:BoundColumn     HeaderText="ProductCategories.LBL_LIST_DESCRIPTION" DataField="DESCRIPTION" SortExpression="DESCRIPTION" />
		</Columns>

...
{
	/// <summary>
	///		Summary description for VerifyTables.
	/// </summary>
	public class VerifyTables : SyncControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				...
				try
				{
					litSOURCE_LIST.Text = LoadNames(nodeSource, "Tables", "Table", lblSOURCE_PROVIDER.Text, lblSOURCE_CONNECTION.Text, GetTablesCommand(lblSOURCE_PROVIDER.Text));
				}
				catch(Exception ex)
				{
				...
				try
				{
					litDESTINATION_LIST.Text = LoadNames(nodeDestination, "Tables", "Table", lblDESTINATION_PROVIDER.Text, lblDESTINATION_CONNECTION.Text, GetTablesCommand(lblDESTINATION_PROVIDER.Text));
				}
				catch(Exception ex)
				{
				...
				CompareNames(nodeSource, nodeDestination, "Tables", "Table", ref sbSourceUnique, ref sbDestinationUnique);

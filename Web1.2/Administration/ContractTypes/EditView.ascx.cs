...
namespace SplendidCRM.Administration.ContractTypes
{
	...
	public class EditView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Save" || e.CommandName == "SaveNew" )
			{
				if ( Page.IsValid )
				{
					try
					{
						SqlProcs.spCONTRACT_TYPES_Update(
							ref gID
							, txtNAME.Text
		...
		{
			Utils.SetPageTitle(Page, L10n.Term("ContractTypes.LBL_NAME"));
							...
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								...
								using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
								{
									if ( rdr.Read() )
									{
										...
										ctlListHeader .Title = L10n.Term("ContractTypes.LBL_NAME") + " " + txtNAME.Text;

	...
	public class Users : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gUSER_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spROLES_USERS_Delete(gID, gUSER_ID);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyString(txtUSER_ID.Value) )
			{
				try
				{
					SqlProcs.spROLES_USERS_MassUpdate(gID, txtUSER_ID.Value);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                  " + ControlChars.CrLf
				     + "  from vwROLES_USERS      " + ControlChars.CrLf
...
#if DEBUG
					Page.RegisterClientScriptBlock("vwROLES_USERS", Sql.ClientScriptBlock(cmd));
#endif
					try
					{

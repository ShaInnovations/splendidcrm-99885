			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "PRODUCT_TEMPLATES");
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "PRODUCT_TEMPLATES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							//SqlProcs.spPRODUCT_TEMPLATES_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

		...
		<ItemTemplate>
			<nobr>
				<asp:HyperLink NavigateUrl='<%# DataBinder.Eval(Container.DataItem, "RELATIVE_PATH") + "view.aspx?ID=" + DataBinder.Eval(Container.DataItem, "ITEM_ID") %>' 
					...
					<asp:Image ImageUrl='<%# Session["themeURL"] + "images/" + DataBinder.Eval(Container.DataItem, "IMAGE_NAME") + ".gif" %>' 
						AlternateText='<%# DataBinder.Eval(Container.DataItem, "ITEM_SUMMARY") %>' BorderWidth="0" Align="absmiddle" Width="16" Height="16" Runat="server" />
					</asp:HyperLink>&nbsp;
				<asp:HyperLink NavigateUrl='<%# DataBinder.Eval(Container.DataItem, "RELATIVE_PATH") + "view.aspx?ID=" + DataBinder.Eval(Container.DataItem, "ITEM_ID") %>' 
					Text='<%# DataBinder.Eval(Container.DataItem, "ITEM_SUMMARY") %>' 
					title='<%# "[" + L10n.Term(".LBL_ALT_HOT_KEY") + DataBinder.Eval(Container.DataItem, "ROW_NUMBER") + "]" %>' 
					AccessKey='<%# DataBinder.Eval(Container.DataItem, "ROW_NUMBER") %>' CssClass="lastViewLink" Runat="server" />&nbsp;
			</nobr>
		</ItemTemplate>
		<FooterTemplate />

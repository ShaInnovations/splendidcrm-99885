...
<div id="divListView">
	...
	<SplendidCRM:ModuleHeader ID="ctlModuleHeader" Module="Reports" Title=".moduleList.Home" EnablePrint="true" EnableHelp="true" Runat="Server" />
	<%@ Register TagPrefix="SplendidCRM" Tagname="ListHeader" Src="~/_controls/ListHeader.ascx" %>

	<table width="100%" border="0" cellpadding="0" cellspacing="0" Visible="<%# !PrintView %>" runat="server">
		<tr>
			<td style="padding-bottom: 2px;">
				<asp:Button CommandName="Reports.Create" OnCommand="Page_Command" CssClass="button" Text='<%# "  " + L10n.Term("Reports.LBL_CREATE_BUTTON_LABEL") + "  " %>' title='<%# L10n.Term("Reports.LBL_CREATE_BUTTON_LABEL") %>' Runat="server" />
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-HorizontalAlign="Right" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton CommandName="Reports.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_DELETE") %>' ImageUrl='<%# Session["themeURL"] + "images/delete_inline.gif" %>' BorderWidth="0" Width="12" Height="12" ImageAlign="AbsMiddle" Runat="server" />
						<asp:LinkButton  CommandName="Reports.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_DELETE") %>' Runat="server" />
					</span>
					&nbsp;
	...
	<SplendidCRM:SplendidGrid id="grdPublished" Width="100%" CssClass="listView"
		...
		<Columns>
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-HorizontalAlign="Right" ItemStyle-Wrap="false">
				<ItemTemplate>
					<span id="Span1" visible='<%# Security.IS_ADMIN || (Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID")) == Security.USER_ID) %>' runat="server">
						<asp:HyperLink NavigateUrl='<%# "~/Reports/ExportRDL.aspx?id=" + DataBinder.Eval(Container.DataItem, "ID") %>' Target="_blank" AlternateText='<%# L10n.Term(".LBL_EXPORT") %>' Visible='<%# nACLACCESS_Export >= 0 %>' Runat="server">
							<asp:Image ImageUrl='<%# Session["themeURL"] + "images/export.gif" %>' BorderWidth="0" Width="10" Height="10" Runat="server" />
						</asp:HyperLink>
						&nbsp;
					</span>
					<asp:ImageButton CommandName="Reports.View" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Reports.LNK_VIEW") %>' ImageUrl='<%# Session["themeURL"] + "images/view_inline.gif" %>' BorderWidth="0" Width="12" Height="12" ImageAlign="AbsMiddle" Runat="server" />
					<asp:LinkButton  CommandName="Reports.View" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term("Reports.LNK_VIEW") %>' Runat="server" />
					&nbsp;
					<span visible='<%# Security.IS_ADMIN || (Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID")) == Security.USER_ID) %>' runat="server">

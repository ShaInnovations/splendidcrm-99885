			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "EMAILS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spEMAILS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "EMAILS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spEMAILS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

	...
	public class Opportunities : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gOPPORTUNITY_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spEMAILS_CONTACTS_Delete(gID, gOPPORTUNITY_ID);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gOPPORTUNITY_ID) )
			{
				try
				{
					SqlProcs.spEMAILS_CONTACTS_Update(gID, gOPPORTUNITY_ID);
					Response.Redirect("view.aspx?ID=" + gID.ToString());
				}
				catch(Exception ex)
				{
...
#if DEBUG
					Page.RegisterClientScriptBlock("vwEMAILS_OPPORTUNITIES", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("Emails.Opportunities");
		}
		#endregion
	}
}

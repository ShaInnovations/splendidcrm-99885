			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Emails.NTC_REMOVE_INVITEE") %>')">
						<asp:ImageButton CommandName="Opportunities.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "OPPORTUNITY_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' ImageUrl='<%# Session["themeURL"] + "images/delete_inline.gif" %>' BorderWidth="0" Width="12" Height="12" ImageAlign="AbsMiddle" Runat="server" />
						<asp:LinkButton  CommandName="Opportunities.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "OPPORTUNITY_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>

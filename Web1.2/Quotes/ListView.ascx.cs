...
namespace SplendidCRM.Quotes
{
	...
	public class ListView : SplendidControl
	{
			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "QUOTES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							// 07/09/2006 Paul.  The date conversion was moved out of the MassUpdate control. 
							//SqlProcs.spQUOTES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.ACCOUNT_ID, ctlMassUpdate.STATUS, T10n.ToServerTime(ctlMassUpdate.START_DATE), T10n.ToServerTime(ctlMassUpdate.END_DATE), T10n.ToServerTime(ctlMassUpdate.COMPANY_SIGNED_DATE), T10n.ToServerTime(ctlMassUpdate.CUSTOMER_SIGNED_DATE));
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "QUOTES");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spQUOTES_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
		...
		{
			//
			// CODEGEN: This Quote is required by the ASP.NET Web Form Designer.
			//
			InitializeComponent();
			base.OnInit(e);
		}

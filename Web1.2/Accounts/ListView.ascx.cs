			...
			try
			{
				...
				else if ( e.CommandName == "MassUpdate" )
				{
					...
					if ( arrID != null )
					{
						...
						sIDs = Utils.FilterByACL(m_sMODULE, "edit", arrID, "ACCOUNTS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spACCOUNTS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.ACCOUNT_TYPE, ctlMassUpdate.INDUSTRY);
				...
				else if ( e.CommandName == "MassDelete" )
				{
					...
					{
						string sIDs = Utils.ValidateIDs(arrID);
						sIDs = Utils.FilterByACL(m_sMODULE, "delete", arrID, "ACCOUNTS");
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spACCOUNTS_MassDelete(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}

<%@ Page language="c#" Codebehind="PopupMultiSelect.aspx.cs" AutoEventWireup="false" Inherits="SplendidCRM.Accounts.PopupMultiSelect" %>
			...
			if ( document.all[i].name == 'chkMain' )
			{
				if ( document.all[i].checked )
				{
					if ( sAccounts.length > 0 )
						sAccounts += ',';
					sAccounts += document.all[i].value;
				}
			}
		}
		window.opener.ChangeAccount(sAccounts, '');
		window.close();
	}
	else
	{
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-bottom: 2px;">
				<input id="btnSelect" type="button" class="button" onclick="SelectAccounts();" value='<%= "  " + L10n.Term("Accounts.LBL_SELECT_CHECKED_BUTTON_LABEL") + "  " %>' title='<%= L10n.Term("Accounts.LBL_SELECT_CHECKED_BUTTON_TITLE") %>' />
				<input id="btnCancel" type="button" class="button" onclick="CancelAccounts();" value='<%= "  " + L10n.Term(".LBL_DONE_BUTTON_LABEL"                  ) + "  " %>' title='<%= L10n.Term(".LBL_DONE_BUTTON_TITLE"                  ) %>' AccessKey='<%= L10n.Term(".LBL_DONE_BUTTON_KEY") %>' />
	...
	<SplendidCRM:SplendidGrid id="grdMain" Width="100%" CssClass="listView"
		...
		<Columns>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="2%">
				<ItemTemplate>
					<input name="chkMain" id="ACCOUNT_ID_<%# DataBinder.Eval(Container.DataItem, "ID") %>" class="checkbox" type="checkbox" value="<%# DataBinder.Eval(Container.DataItem, "ID") %>" />
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:BoundColumn     HeaderText="Accounts.LBL_LIST_ACCOUNT_NAME"  DataField="NAME"        SortExpression="NAME"        ItemStyle-Width="64%" />
			<asp:BoundColumn     HeaderText="Accounts.LBL_LIST_CITY"          DataField="CITY"        SortExpression="CITY"        ItemStyle-Width="34%" />
		</Columns>

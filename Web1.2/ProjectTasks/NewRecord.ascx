...
<div id="divNewRecord">
	...
	<table border="0" cellpadding="0" cellspacing="0" width="180" class="leftColumnModuleHead">
		<tr>
			...
			<th style="background-image: url(<%= Sql.ToString(Session["themeURL"]) %>images/moduleTab_middle.gif);" width="100%"><%= L10n.Term("ProjectTask.LBL_NEW_FORM_TITLE") %></th>
	...
	<table border="0" cellpadding="3" cellspacing="0" width="180">
		<tr>
			<td align="left" class="leftColumnModuleS3">
				<p>
				<%= L10n.Term("ProjectTask.LBL_NAME") %>&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br>
				<asp:TextBox ID="txtNAME" Runat="server" /><br>
				<%= L10n.Term("ProjectTask.LBL_PARENT_ID") %>&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br>
				...
				<input ID="btnSelectProject" type="button" class="button" onclick="return ProjectPopup();" title="<%# L10n.Term(".LBL_SELECT_BUTTON_TITLE") %>" accessKey="<%# L10n.Term(".LBL_SELECT_BUTTON_KEY") %>" value="<%# L10n.Term(".LBL_SELECT_BUTTON_LABEL") %>" /><br>
				...
				<CustomValidators:RequiredFieldValidatorForHiddenInputs ID="reqPROJECT_ID" ControlToValidate="txtPROJECT_ID" ErrorMessage='<%# L10n.Term(".ERR_REQUIRED_FIELD") %>' CssClass="required" Enabled="false" EnableClientScript="false" EnableViewState="false" Runat="server" />
			</td>
		</tr>
	</table>

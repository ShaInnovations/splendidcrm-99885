...
namespace SplendidCRM.ProjectTasks
{
	...
	/// </summary>
	public class MyProjectTasks : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                                   " + ControlChars.CrLf
				     + "  from vwPROJECT_TASKS_MyList              " + ControlChars.CrLf
				     + " where ASSIGNED_USER_ID = @ASSIGNED_USER_ID" + ControlChars.CrLf
				     + " order by DATE_DUE, PRIORITY desc          " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Sql.AddParameter(cmd, "@ASSIGNED_USER_ID", Security.USER_ID);
#if DEBUG
					Page.RegisterClientScriptBlock("vwPROJECT_TASKS_MyList", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("ProjectTask.MyProjectTasks");
		}
		#endregion
	}
}

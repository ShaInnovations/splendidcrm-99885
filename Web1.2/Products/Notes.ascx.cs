...
namespace SplendidCRM.Products
{
	...
	public class Notes : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select *                       " + ControlChars.CrLf
				     ...
				     + " where PRODUCT_ID = @PRODUCT_ID" + ControlChars.CrLf
				     + " order by DATE_MODIFIED asc    " + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					Sql.AddParameter(cmd, "@PRODUCT_ID", gID);
#if DEBUG
					Page.RegisterClientScriptBlock("vwPRODUCTS_NOTES", Sql.ClientScriptBlock(cmd));
#endif
					try
					{
		...
		private void InitializeComponent()
		{
			...
			grdMain.DynamicColumns("Products.Notes");
		}
		#endregion
	}
}

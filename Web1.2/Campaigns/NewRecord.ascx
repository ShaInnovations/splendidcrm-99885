...
<div id="divNewRecord">
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="leftColumnModuleHead">
		<tr>
			...
			<th style="background-image: url(<%= Sql.ToString(Session["themeURL"]) %>images/moduleTab_middle.gif);" width="100%"><%= L10n.Term("Campaigns.LBL_NEW_FORM_TITLE") %></th>
	...
	<table width="100%" cellpadding="3" cellspacing="0" border="0">
		<tr>
			<td align="left" class="leftColumnModuleS3">
				<p>
				<%= L10n.Term("Campaigns.LBL_NAME") %>&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br>
				<asp:TextBox ID="txtNAME" size="27" MaxLength="255" Runat="server" /><br>
				<%= L10n.Term("Campaigns.LBL_CAMPAIGN_STATUS") %>&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br>
				<asp:DropDownList ID="lstSTATUS" DataValueField="NAME" DataTextField="DISPLAY_NAME" Runat="server" /><br>
				<%= L10n.Term("Campaigns.LBL_CAMPAIGN_END_DATE") %>&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br>
				...
				<%= L10n.Term("Campaigns.LBL_CAMPAIGN_TYPE") %>&nbsp;<asp:Label CssClass="required" Text='<%# L10n.Term(".LBL_REQUIRED_SYMBOL") %>' Runat="server" /><br>
				...
				<CustomValidators:RequiredFieldValidatorForDatePicker   ID="reqEND_DATE"      ControlToValidate="ctlEND_DATE"      CssClass="required" EnableClientScript="false" EnableViewState="false" Enabled="false" Runat="server" />
				<CustomValidators:DatePickerValidator                   ID="valEND_DATE"      ControlToValidate="ctlEND_DATE"      CssClass="required" EnableClientScript="false" EnableViewState="false" Enabled="false" Runat="server" />
